...
function publishNews(aRay) {
  for(var i = 0; i < aRay.length; i++) {
    ...
    newsDiv.innerHTML += `<div id="news_article--${i}" class="newsDiv"><img id="newsImage" src="${article.urlToImage}"><h3>${headline.title}</h3><h4>${headline.source_name}</h4><p>${headline.description}</p><a href="${headline.url}" target="_blank">Read more&hellip;</a><a href="#">Save</a></div>`;
